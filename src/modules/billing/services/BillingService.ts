...
export const BillingService = {
  ...
  async listServices(): Promise<ProfessionalService[]> {
    const { data, error } = await supabase.from("professional_services").select("*").order("position");
    if (error) throw error;
    return data;
  },

  async updateSubscription(userId: string, input: Pick<Subscription, "plan_id" | "status" | "billing_interval" | "current_period_end" | "notes">) {
  ...
  async updatePlan(id: string, input: Partial<Pick<Plan, "name" | "description" | "price_cents" | "active" | "limits" | "features">>) {
