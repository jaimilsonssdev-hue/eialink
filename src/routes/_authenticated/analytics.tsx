...
import { Eye, MousePointerClick, MessageCircle, QrCode, Instagram, Smartphone, Monitor } from "lucide-react";
...
function AnalyticsPage() {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!bio) return null;
      const { data: events } = await supabase.from("analytics_events").select("*").eq("bio_page_id", bio.id).order("created_at", { ascending: false }).limit(500);
  ...
  const byDevice = data.events.reduce<Record<string, number>>((acc, e) => { const k = e.device ?? "outro"; acc[k] = (acc[k] ?? 0) + 1; return acc; }, {});
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    days.push({ key, label: d.toLocaleDateString("pt-BR", { weekday: "short" }).replace(".", ""), count: 0 });
  }
  data.events.forEach((e) => {
    const key = new Date(e.created_at).toISOString().slice(0, 10);
    const day = days.find((d) => d.key === key); if (day) day.count++;
  });
  const maxCount = Math.max(1, ...days.map((d) => d.count));

  const conversionRate = byType.view ? Math.round(((byType.whatsapp_click ?? 0) + (byType.pix_click ?? 0)) / byType.view * 100) : 0;

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Analytics</h1>
        <p className="mt-2 text-muted-foreground">Acompanhe o desempenho da sua página em tempo real.</p>
      </div>

      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Visualizações" value={byType.view ?? 0} icon={Eye} color="var(--brand-cyan)" />
        <StatCard label="Cliques em links" value={byType.link_click ?? 0} icon={MousePointerClick} color="var(--brand-violet)" />
        <StatCard label="Cliques WhatsApp" value={byType.whatsapp_click ?? 0} icon={MessageCircle} color="var(--brand-lime)" />
        <StatCard label="Taxa de conversão" value={`${conversionRate}%`} icon={QrCode} color="var(--brand-amber)" sub="Views → Contato" />
      </div>
        ...
        <div className="mt-6 flex items-end justify-between gap-2 h-40">
          {days.map((d) => (
            <div key={d.key} className="flex-1 flex flex-col items-center gap-2">
              ...
              <div className="w-full rounded-t-lg transition-all min-h-[4px]" style={{
        ...
        <div className="card-surface">
          ...
          <div className="space-y-3">
            {Object.entries(byType).sort((a, b) => b[1] - a[1]).map(([k, v]) => {
              const meta = EVENT_LABELS[k] ?? { label: k, icon: Eye, color: "var(--muted-foreground)" };
              const Icon = meta.icon;
              const pct = Math.round((v / total) * 100);
              return (
                <div key={k}>
                  <div className="flex items-center justify-between text-sm">
                    <span className="flex items-center gap-2"><Icon className="h-4 w-4" style={{ color: meta.color }} /> {meta.label}</span>
            ...
            {total === 0 && <p className="text-sm text-muted-foreground">Ainda não há dados. Compartilhe sua página.</p>}
          </div>
        </div>
          ...
          <div className="space-y-3">
            {Object.entries(byDevice).sort((a, b) => b[1] - a[1]).map(([k, v]) => {
              ...
              return (
                <div key={k}>
                  <div className="flex items-center justify-between text-sm">
                    <span className="flex items-center gap-2 capitalize"><Icon className="h-4 w-4 text-[color:var(--brand-cyan)]" /> {k}</span>
