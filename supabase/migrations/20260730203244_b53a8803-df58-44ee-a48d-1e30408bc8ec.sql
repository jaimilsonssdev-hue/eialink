ALTER TABLE public.bio_pages ADD COLUMN IF NOT EXISTS whatsapp_message TEXT;
DO $$ BEGIN
  ALTER TABLE public.bio_pages ADD CONSTRAINT bio_pages_whatsapp_message_length CHECK (whatsapp_message IS NULL OR char_length(whatsapp_message) <= 1000);
...
DROP TRIGGER IF EXISTS professional_services_updated ON public.professional_services;
...
GRANT SELECT, INSERT, UPDATE, DELETE ON public.plans, public.subscriptions, public.professional_services TO authenticated;
