  ...
  if (!pages.data?.length) {
    return (
      <div className="premium-page">
        <div className="premium-panel text-center py-14"><ShoppingBag className="mx-auto h-8 w-8 text-[color:var(--primary)]" /><h1 className="mt-4 text-xl font-semibold">Crie seu primeiro Biolink</h1><p className="mt-2 text-sm text-muted-foreground">Seu catálogo pertence a uma página.</p><Link className="btn-primary mt-5" to="/pages"><PackagePlus className="h-4 w-4" /> Criar Biolink</Link></div>
      </div>
    );
  }
  ...
  return (
    <div className="premium-page">
      <header className="catalog-workspace-header">
        <div className="premium-page-heading"><p className="eyebrow">Produtos e serviços</p><h1>Catálogo</h1><p>Mostre o que você oferece e leve os clientes direto para o WhatsApp.</p></div>
        <div className="catalog-workspace-actions">
          ...
          <button className="btn-primary" onClick={() => void saveCatalog()} disabled={saving || !changed}>{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}{saving ? "Salvando" : changed ? "Salvar catálogo" : "Salvo"}</button>
        </div>
      </header>
      <div className="catalog-workspace-grid">
        ...
        <aside className="catalog-workspace-tip"><ShoppingBag className="h-5 w-5" /><b>Catálogo da sua página</b><p>Imagens, ordem e visibilidade ficam vinculadas somente a este Biolink.</p><Link to="/builder" search={{ page: selectedPageId }} className="btn-secondary">Personalizar página</Link></aside>
      </div>
