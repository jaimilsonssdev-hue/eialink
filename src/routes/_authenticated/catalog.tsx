...
function CatalogPage() {
  ...
  if (!access.data?.features.catalog) {
    return (
      <div className="premium-page">
        <div className="premium-page-heading">
          <p className="eyebrow">Catálogo</p>
          <h1>Transforme sua página em vitrine</h1>
          <p>Produtos, serviços, imagens e botões de pedido ficam disponíveis no Eialink Pro.</p>
        </div>
        <div className="mt-6 max-w-2xl">
          <UpgradePrompt
            title="Seu catálogo fica pronto no Eialink Pro"
            description="Apresente produtos e serviços com imagem, preço, botão e ordem personalizada."
          />
        </div>
      </div>
    );
  ...
  if (!pages.data?.length) {
    return (
      <div className="premium-page">
        <div className="premium-panel text-center py-14">
          <ShoppingBag className="mx-auto h-8 w-8 text-[color:var(--primary)]" />
          <h1 className="mt-4 text-xl font-semibold">Crie seu primeiro Biolink</h1>
          <p className="mt-2 text-sm text-muted-foreground">Seu catálogo pertence a uma página.</p>
          <Link className="btn-primary mt-5" to="/pages">
            <PackagePlus className="h-4 w-4" /> Criar Biolink
          </Link>
        </div>
      </div>
    );
    ...
    <div className="premium-page catalog-premium-page">
      <header className="catalog-workspace-header">
        <div className="premium-page-heading">
          ...
          <h1>Catálogo</h1>
          <p>Organize o que você oferece e leve seus clientes direto para a próxima ação.</p>
        </div>
        <div className="catalog-workspace-actions">
          <label>
            <span>Biolink ativo</span>
            <select
          ...
          >
            ...
            {saving ? "Salvando" : changed ? "Salvar alterações" : "Tudo salvo"}
          </button>
        </div>
      </header>
      <div className="catalog-workspace-intro">
        <ShoppingBag className="h-5 w-5" />
        <p>
          <b>Dica rápida:</b> use uma imagem quadrada, um título direto e uma ação clara. As
          mudanças aparecem no seu Biolink após salvar.
        </p>
      </div>
      <div className="catalog-workspace-grid">
        <section className="premium-panel">
          {products.isLoading ? (
            ...
            <CatalogEditor items={items} onChange={setItems} />
          )}
        </section>
        <aside className="catalog-workspace-tip">
          <ShoppingBag className="h-5 w-5" />
      ...
      {message && (
        <p className="catalog-workspace-message" role="status">
          {message}
        </p>
      )}
    </div>
