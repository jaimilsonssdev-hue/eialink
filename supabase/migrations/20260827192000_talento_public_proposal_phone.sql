drop function if exists public.get_talento_public_proposal(text);
create or replace function public.get_talento_public_proposal(p_token text)
returns table (
  proposal_id uuid, company_name text, phone text, title text, objective text, scope text,
  deliverables text, deadline text, investment_cents integer, terms text, valid_until date, status text
)
language sql security definer set search_path = public as $$
  select p.id, l.company_name, l.phone, p.title, p.objective, p.scope, p.deliverables,
  ...
  where p.public_token = p_token limit 1;
$$;
